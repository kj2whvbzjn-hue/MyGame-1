...
namespace GuildAdventure.Tests.EditMode
{
 public sealed class CoverAttackPipelineTests
 {
  ...
  [Test] public void CoveredAttackBuildsProposalForProtectorButKeepsOriginalReservationTarget(){var s=Snapshot();var rng=new Rng();var seen=new List<string>();var reservation=Reservation("SINGLE",new List<string>{"T"});var cover=new CoverContract{id="CV",protectorId="C",protectedActorId="T",lifetime=CoverLifetimeKind.PERSISTENT,active=true};var r=MultiTargetBattleExecutor.Execute(s,new MultiTargetAttackRequest{reservation=reservation,buildAttack=id=>{seen.Add(id);var p=Attack();p.baseDamage=id=="C"?17:3;return p;},coverContracts=new[]{cover}},rng,rng,rng);Assert.IsTrue(r.ok,r.reason);CollectionAssert.AreEqual(new[]{"C"},seen);CollectionAssert.AreEqual(new[]{"T"},reservation.fixedTargetIds);Assert.AreSame(reservation,r.targetResults[0].reservation);Assert.AreEqual(83,s.actors[2].hp);}
 }
}
#endif
