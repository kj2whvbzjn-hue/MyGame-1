...
namespace GuildAdventure.Tests.EditMode
{
 ...
 {
  static ActionGaugeSettings Gauge()=>new ActionGaugeSettings{maxGauge=100,aiReevaluationRatio=.1,successfulActionConsumeRatio=1,failedExecutionConsumeRatio=.5,agiGaugeCoefficient=1,actionSpeedMultiplier=1};
  [Test] public void ActionNode_BuildsC02ReadySkillRequest()
  {
   ...
   var c=new FormalAiBattleContext{program=p,execution=new FormalAiExecutionContext{maxSteps=16},actionGaugeSettings=Gauge(),resolveSkill=n=>new SkillDefinition{id="S"},resolveTargetCandidates=n=>new[]{"B"},buildAttack=(n,sk,t)=>new BattleAttackProposal{hitCount=1,damageType=DamageType.Physical,accuracy=100,baseDamage=1}};var r=FormalAiBattleBridge.Decide(s,a,c);Assert.IsTrue(r.ok,r.reason);Assert.AreEqual("A",r.request.attack.sourceId);Assert.AreEqual("B",r.request.attack.targetId);Assert.AreEqual("S",r.request.attack.skillId);Assert.AreEqual("AI:7:A:S",r.request.attack.reservationId);
  }
  [Test] public void NotReadyGauge_DoesNotEvaluateGraph(){var s=new BattleSnapshotSaveRecord();var a=new BattleActorSaveRecord{actorId="A",alive=true,hp=10,actionGauge=90};s.actors.Add(a);var r=FormalAiBattleBridge.Decide(s,a,new FormalAiBattleContext{actionGaugeSettings=Gauge()});Assert.IsFalse(r.ok);Assert.AreEqual("AI_BATTLE_GAUGE_NOT_READY",r.reason);}
  [Test] public void MissingGaugeSettings_IsRejectedBeforeGraph(){var s=new BattleSnapshotSaveRecord();var a=new BattleActorSaveRecord{actorId="A",alive=true,hp=10,actionGauge=100};s.actors.Add(a);var r=FormalAiBattleBridge.Decide(s,a,new FormalAiBattleContext());Assert.IsFalse(r.ok);Assert.AreEqual("AI_BATTLE_GAUGE_SETTINGS_MISSING",r.reason);}
 }
}
#endif
