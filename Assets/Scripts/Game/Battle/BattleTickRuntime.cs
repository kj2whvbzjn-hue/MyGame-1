...
namespace GuildAdventure.Game.Battle
{
    ...
    public static class BattleTickRuntime
    {
        ...
        {
            if(snapshot==null)return Fail("BATTLE_TICK_SNAPSHOT_MISSING");var validation=BattleSnapshotValidation.Validate(snapshot);if(validation!=null)return Fail(validation);var result=new BattleTickResult{tick=snapshot.tick};var gaugeSettings=new ActionGaugeSettings();
            foreach(var id in snapshot.fixedActorOrder)
            {
                var actor=snapshot.actors.Find(x=>x.actorId==id);if(actor==null)continue;TickPeriodic(actor,result);TickCooldowns(actor);ExpireEffects(actor,result);
        ...
        static void TickPeriodic(BattleActorSaveRecord actor,BattleTickResult result)
        {
            ...
            {if(e.kind==EffectLifecycleKind.DOT.ToString()&&actor.alive&&actor.hp>0){var damage=EffectLifecycleRuntime.DotDamage(e.value);var hp=DamageDefense.CommitHp(actor.hp,damage);actor.hp=hp.hpAfter;actor.alive=actor.hp>0;result.dotSources.Add(e.instanceId);}e.remainingTicks--;}
        }
