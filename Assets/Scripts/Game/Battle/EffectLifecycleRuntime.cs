...
namespace GuildAdventure.Game.Battle
{
 ...
 {
  public string instanceId,sourceId,effectId,refreshRule,snapshotPolicy,dispelCategory;
  ...
  public bool removable=true,protectedEffect,normalCleanseEligible,actionDisabled,removeOnDeath=true,removeOnBattleEnd=true;
 }
 ...
 public static class EffectLifecycleRuntime
 {
  ...
  public static EffectApplyResult Apply(BattleActorSaveRecord actor,EffectApplyRequest request)
  {
   ...
   if(rule==EffectStackRule.UNIQUE_REFRESH&&same.Count>0){var current=same[0];foreach(var duplicate in same.Skip(1))actor.appliedEffects.Remove(duplicate);current.remainingTicks=duration;current.value=request.value;current.sourceId=request.sourceId;current.appliedTick=request.appliedTick;current.sequence=request.sequence;CopyLifecycle(current,request);return new EffectApplyResult{ok=true,applied=current,effectiveDurationTicks=duration};}
   ...
   var row=new AppliedEffectSaveRecord{instanceId=request.instanceId,sourceId=request.sourceId,effectId=request.effectId,kind=request.kind.ToString(),remainingTicks=duration,value=request.value,appliedTick=request.appliedTick,sequence=request.sequence};CopyLifecycle(row,request);actor.appliedEffects.Add(row);return new EffectApplyResult{ok=true,applied=row,effectiveDurationTicks=duration};
  }
  static void CopyLifecycle(AppliedEffectSaveRecord row,EffectApplyRequest request){row.refreshRule=request.refreshRule;row.snapshotPolicy=request.snapshotPolicy;row.dispelCategory=request.dispelCategory;row.removeOnDeath=request.removeOnDeath;row.removeOnBattleEnd=request.removeOnBattleEnd;row.removable=request.removable;row.protectedEffect=request.protectedEffect;row.normalCleanseEligible=request.normalCleanseEligible;row.actionDisabled=request.actionDisabled;}
  ...
  public static void CleanupOnDeath(BattleActorSaveRecord actor){if(actor==null)return;var removed=new HashSet<string>((actor.appliedEffects??new List<AppliedEffectSaveRecord>()).Where(x=>x!=null&&x.removeOnDeath).Select(x=>x.instanceId),StringComparer.Ordinal);actor.appliedEffects?.RemoveAll(x=>x!=null&&x.removeOnDeath);actor.barrierLayers?.RemoveAll(x=>x==null||removed.Contains(x.id));}
  public static void CleanupBattleEnd(BattleSnapshotSaveRecord snapshot){if(snapshot==null)return;foreach(var actor in snapshot.actors??new List<BattleActorSaveRecord>()){if(actor==null)continue;var removed=new HashSet<string>((actor.appliedEffects??new List<AppliedEffectSaveRecord>()).Where(x=>x!=null&&x.removeOnBattleEnd).Select(x=>x.instanceId),StringComparer.Ordinal);actor.appliedEffects?.RemoveAll(x=>x!=null&&x.removeOnBattleEnd);actor.barrierLayers?.RemoveAll(x=>x==null||removed.Contains(x.id));actor.cooldowns?.Clear();actor.cast=null;}}
  ...
  static IEnumerable<AppliedEffectSaveRecord> RemovableStatuses(BattleActorSaveRecord actor,string effectId,bool normalCleanse)=>(actor?.appliedEffects??new List<AppliedEffectSaveRecord>()).Where(x=>x!=null&&!x.consumed&&string.Equals(x.dispelCategory??x.kind,"STATUS",StringComparison.OrdinalIgnoreCase)&&x.removable&&!x.protectedEffect&&(effectId==null||x.effectId==effectId)&&(!normalCleanse||x.normalCleanseEligible)).OrderBy(x=>x.appliedTick).ThenBy(x=>x.sequence).ThenBy(x=>x.instanceId,StringComparer.Ordinal);
