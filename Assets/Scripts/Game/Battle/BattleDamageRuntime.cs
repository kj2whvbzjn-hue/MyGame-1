...
namespace GuildAdventure.Game.Battle
{
 ...
 {
  public static BattleDamageResult Commit(BattleActorSaveRecord target,int damage,Func<int,int,int?> fatalResolver=null,bool useBarrier=true)=>Commit(target,damage,null,fatalResolver,useBarrier);
  public static BattleDamageResult Commit(BattleActorSaveRecord target,int damage,IEnumerable<BarrierLayer> barrierState,Func<int,int,int?> fatalResolver=null,bool useBarrier=true){if(target==null)throw new ArgumentNullException(nameof(target));var requested=Math.Max(0,damage);var ordered=(target.barrierLayers??new List<BarrierLayerSaveRecord>()).Where(x=>x!=null).OrderBy(x=>x.appliedTick).ThenBy(x=>x.sequence).ThenBy(x=>x.id,StringComparer.Ordinal).ToList();var runtime=barrierState!=null?barrierState.Select(x=>new BarrierLayer{id=x.id,remaining=x.remaining}).ToList():ordered.Select(x=>new BarrierLayer{id=x.id,remaining=x.remaining}).ToList();var barrier=useBarrier?DamageDefense.ConsumeBarrierFifo(requested,runtime):new BarrierResult{hpDamageCandidate=requested,layers=runtime};if(ordered.Count>0)Persist(target,ordered,barrier.layers);var hp=DamageDefense.CommitHp(target.hp,barrier.hpDamageCandidate,fatalResolver);target.hp=hp.hpAfter;target.alive=target.hp>0;return new BattleDamageResult{requestedDamage=requested,barrierAbsorbed=barrier.absorbed,hpDamageCandidate=barrier.hpDamageCandidate,hpBefore=hp.hpBefore,hpAfter=hp.hpAfter,actualHpLoss=hp.actualHpLoss,fatalCandidate=hp.fatalCandidate,fatalPrevented=hp.fatalPrevented,died=hp.hpBefore>0&&hp.hpAfter<=0,remainingBarriers=barrier.layers};}
