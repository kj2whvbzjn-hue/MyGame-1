...
namespace GuildAdventure.Game.AI
{
 ...
 {
  public FormalAiProgram program;public FormalAiExecutionContext execution;public IRandomSource tieRng;public ActionGaugeSettings actionGaugeSettings;
 ...
 {
  public static FormalAiExecutionResult Reevaluate(FormalAiBattleContext context){if(context==null)return new FormalAiExecutionResult{ok=false,reason="AI_BATTLE_CONTEXT_INVALID"};return FormalAiV2Executor.Execute(context.program,context.execution);}
  ...
  {
   if(snapshot==null||actor==null||context==null)return Fail("AI_BATTLE_CONTEXT_INVALID");if(!actor.alive||actor.hp<=0)return Fail("AI_BATTLE_ACTOR_DEAD");if(context.actionGaugeSettings==null||context.actionGaugeSettings.Validate()!=null)return Fail("AI_BATTLE_GAUGE_SETTINGS_MISSING");if(actor.actionGauge<context.actionGaugeSettings.maxGauge)return Fail("AI_BATTLE_GAUGE_NOT_READY");
   var graph=Reevaluate(context);if(!graph.ok)return Fail(graph.reason,graph.visitedNodeIds);var node=graph.action;if(node==null)return Fail("AI_BATTLE_ACTION_MISSING",graph.visitedNodeIds);var skill=context.resolveSkill?.Invoke(node);if(skill==null)return Fail("AI_BATTLE_SKILL_MISSING",graph.visitedNodeIds);var target=AiDecisionTargetRuntime.Select(snapshot,context.resolveTargetCandidates?.Invoke(node),context.tieRng);if(!target.ok)return Fail(target.reason,graph.visitedNodeIds);var attack=context.buildAttack?.Invoke(node,skill,target.targetId);if(attack==null)return Fail("AI_BATTLE_ATTACK_PROPOSAL_MISSING",graph.visitedNodeIds);
